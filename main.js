...
d3.select("body").append("h2").text("Starting Part 2! Were Learning D3")
...
d3.select("#dynamic-content").append("p").text("Look at this!")
...
d3.csv("../data/schools.csv", rowconverter, function(schooldata) {
    console.log(schooldata);
    handledata(schooldata);
});
...
function handledata(data) {
    ...
    let filteredData = data.filter(function(row) {
        //only look at datamatch schools
        return row.eu === "TRUE";
    });
...
//   - Use the randomly generated x/y coordinates for each school from the dataset to position the circles
    svg2 = d3.select("body").append("svg").attr("width",700).attr("height",550)
